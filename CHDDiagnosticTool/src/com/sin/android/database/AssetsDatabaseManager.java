...
public class AssetsDatabaseManager {
	...
	/**
	 * Clear all database file, using when update.
	 * 
	 * @return 是否删除成功!
	 */
	public boolean clearAllDataBaseFile() {
		String path = getDatabaseFilepath();
		File dir = new File(path);
		if (dir.exists() && dir.isDirectory()) {
			Log.i(tag, "clearAllDataBaseFile");
			for (String fl : dir.list()) {
				String filepath = String.format("%s/%s", path, fl);
				File file = new File(filepath);
				if (file.delete() == false) {
					Log.e(tag, String.format("  delete file fail: %s", filepath));
				} else {
					Log.i(tag, String.format("  delete file: %s", filepath));
				}
			}
			return true;
		} else {
			Log.e(tag, String.format("clearAllDataBaseFile fail: %s", path));
			return false;
		}
	}
